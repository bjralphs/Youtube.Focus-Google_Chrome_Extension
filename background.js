...
chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
    if (changeInfo.url && changeInfo.url.includes('https://www.youtube.com/shorts/')) {
      chrome.storage.sync.get('shortsRedirectEnabled', function(data) {
        let enabled = data.shortsRedirectEnabled;
        if (enabled) {
          chrome.tabs.update(tab.id, {url: 'https://www.youtube.com/'});
          // Execute the alert in the context of the content.
          chrome.tabs.executeScript(tab.id, {code: 'window.alert("Youtube Shorts Are Currently Blocked");'});
        }
      });
    }
  });
...
chrome.runtime.onInstalled.addListener(function() {
    chrome.storage.sync.set({
        ...
        if (chrome.runtime.lastError) {
            console.error("Error setting initial values:", chrome.runtime.lastError);
        } else {
